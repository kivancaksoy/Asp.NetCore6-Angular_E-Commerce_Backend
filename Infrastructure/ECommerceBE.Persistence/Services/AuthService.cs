...
namespace ECommerceBE.Persistence.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task PasswordResetAsync(string email)
        {
            ...
            if (user != null)
            {
                //üretilen reset token ',+ gibi kompleks değer taşıyabiliyor, bu değerler taşınırken hata verir.
                string resetToken = await _userManager.GeneratePasswordResetTokenAsync(user);

                ////Üretilen resetToken değeri byte dizisine dönüştürüldü.
                //byte[] tokenBytes = Encoding.UTF8.GetBytes(resetToken);

                ////URL'de taşınabilir(HTTP protokolünde kullanılabilir) şifrelenmiş bir resetToken'a dönüştürüldü.
